...
- **Frontend**: `RepositoryUploads` (📎 attach, removable chips, upload-on-select) shown in TaskCreate (when an existing repo is selected) and TaskDetail. **UI placement changed 2026-07-09** (user feedback — see that day's roadmap entry below): on TaskDetail the 📎 control now lives in the instruction textarea's Markdown toolbar with the chips just above it (GitHub-Issue-comment-box style), replacing the original standalone "Reference files" strip below the topbar. Attachment model (repository-scoped, persists across every task) unchanged.
...
- **Sprint 2 UI fix: file-attach control moved into the instruction toolbar** — User feedback: the standalone "Reference files" strip didn't read as an attach-to-this-message operation, unlike a GitHub Issue comment box. `components/RepositoryUploads.tsx` split into a hook + `AttachFilesButton`/`AttachedFilesChips`; on TaskDetail the 📎 button now sits in the Markdown toolbar (with B/I/code/...) and the chips render just above the textarea. TaskCreate's placement (no per-message toolbar exists there) is unchanged. Operation-only — the repository-scoped, cross-task persistent attachment model is untouched. Verified end-to-end in the browser (upload via the new button, chip placement, DB row still correctly repository-scoped) plus typecheck/build.
- **Ops: fixed upload/task data defaulting under `/tmp` (data-loss risk)** — `backend/.env` had no `UPLOAD_DATA_PATH`, so host-run backends fell back to `/tmp/xolvien/uploads`, which Linux can clear on reboot / via `systemd-tmpfiles`. This had already destroyed two previously uploaded files (DB rows survived, pointing at bytes that no longer existed). Repointed `TASK_DATA_PATH`/`UPLOAD_DATA_PATH` to `var/tasks`/`var/uploads` under the repo and removed the two orphaned `uploads` rows.
