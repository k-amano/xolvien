...
## 手順1｜Claude Code CLI の認証を確認する

Karakuri は Claude Code CLI を使って AI と通信します。
まず、ホストマシンで Claude Code CLI が認証済みかを確認します。

「ターミナル」を開いて以下を入力して `Enter` を押します。
...
保存された `translator.html` をダブルクリックします。ブラウザが開いて翻訳アプリが表示されます。
