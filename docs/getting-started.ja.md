...
> **ヒント｜仕様書の添付：** 仕様書や設計書（Excel / Word / PDF / Markdown）が既にある場合は、**📎** ボタンで添付できます（既存リポジトリを選択したときのこの画面と、タスク詳細画面に表示されます）。添付ファイルは要件の確認や実装の際に Claude が読み取ります。本格的なサンプル仕様書を [`docs/samples/sample-spec.xlsx`](samples/README.md) として提供しています。
