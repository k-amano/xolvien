...
> 仕様書や設計書が既にある場合は、ここで添付できます。手順は下の「手順8-2｜仕様書ファイルを添付する」を参照してください。
...
## 手順8-2｜仕様書ファイルを添付する（任意）

仕様書や設計書（Excel `.xlsx`、Word `.docx`、PDF、Markdown/テキスト）があれば添付できます。添付したファイルは、要件確認や実装の際に Claude が読み取ります。この手順は任意です — まだファイルがなければ飛ばしてかまいません（後からいつでも添付できます）。

ファイルは「タスク」ではなく**リポジトリ（プロジェクト）単位**で添付されます。同じプロジェクトで作る2つ目以降のタスクでも、再アップロードせずに同じ添付ファイルを利用できます。

> **付属のサンプルで試す：** このリポジトリには本格的なサンプル仕様書
> `docs/samples/sample-spec.xlsx`（経費管理 REST API の仕様書）が同梱されています。内容は
> [`docs/samples/README.md`](samples/README.md) を参照してください。

**📎ボタンが表示される場所：**

| 画面 | 場所 |
|---|---|
| タスク作成ダイアログ | 「既存リポジトリを選択」タブ（ステップA）で**リポジトリを選択した後にのみ**、リポジトリのプルダウンの下に「参照ファイル 📎」欄が現れます。「新規リポジトリ追加」タブや「GitHubで作成」タブでは表示されません（添付先のリポジトリがまだ存在しないため）。リポジトリ未選択の状態でも表示されません。 |
| タスク詳細画面 | **指示入力欄の Markdown ツールバー**（`B / I / <> / ``` / — / •` が並ぶ、入力欄のすぐ上の行）の一番右、区切り線の後に 📎 ボタンがあります。GitHub の Issue コメント欄と同じように、添付操作を「入力する場所のすぐそば」に置く構成です。 |

**添付の手順：**

1. **📎** ボタンをクリックします。
2. ファイル選択ダイアログで1つまたは複数のファイルを選びます。選択と同時にアップロードが始まります（別途「確定」操作は不要です）。
3. アップロードが完了すると、**入力欄のすぐ上**に「参照ファイル」というラベル付きでファイル名のチップが表示されます。チップの **×** をクリックすると削除できます。

**どのファイルを参照するかはメッセージごとに選べます。**チップ本体をクリックすると切り替わります。

| 表示 | 意味 |
|---|---|
| **✓ 実線枠** | 次に送るメッセージで参照される（既定。新規アップロードも自動でON） |
| **○ 破線枠・グレー** | 保存はされているが、次のメッセージでは参照されない |

選択したファイルは、Claude を実行するタイミング（要件確認 or 実装実行）で自動的にタスクコンテナへコピーされます。追加の操作は不要です。

参照ファイルを選択して要件確認を始めると、Claude は**最初にそのファイルを読んでから**質問を開始します（ファイルに書いてあることは質問しません）。読んだ証拠が確認できない場合は「AIが参照ファイルを読みませんでした」というエラーが表示され、仕様書を読まないままヒアリングが進むことはありません。ファイルサイズの制限はありません — 大きなファイルは必要な部分だけが読まれます。

> **📎ボタンが見当たらない場合：** 指示入力欄が「Write」タブになっているか（「Preview」タブでは表示されません）、タスクにコンテナが準備できているか確認してください（リポジトリが空・失敗している場合は添付先がありません）。また、古い画面がキャッシュ表示されている可能性もあるため、ブラウザの強制リロード（Ctrl+Shift+R / Cmd+Shift+R）も試してください。
...
「送信」をクリックすると、Claude が**1問ずつ**質問してきます。各質問はチャットカードとして表示され、選択肢ボタンが付いています。
...
**回答方法：** 選択肢ボタンをクリックするか、入力欄に自由に回答を入力して **「回答を送信」** をクリックします。**Enter キーでは送信されません**（改行になります）。

このガイドでは、各質問に以下のように回答してください：

| 質問 | 回答 |
|---|---|
| 使用するプログラミング言語・フレームワークは？ | **「Vanilla HTML/CSS/JS」** ボタンをクリック |
| 翻訳APIはどれを使いますか？ | `https://api.mymemory.translated.net を使ってください（APIキー不要）` と入力 |
| デザインの要望はありますか？ | `シンプルで見やすければOKです` と入力 |

Claude は1問ずつ質問してきます。1つ答えると次の質問が表示されます。

要件が十分に揃ったと判断したら、**「プロンプト生成へ進む」** ボタンをクリックしてプロンプト生成フェーズへ進みます。
...
「確定して実行」をクリックすると、左の黒いログエリアに Claude の動作がリアルタイムで表示されます。ログエリアは**生の表示（コンソールのようなもの）**で、Claude Code CLI の `stream-json` 出力（送ったプロンプト、思考、ツール呼び出し＝ファイル読み込み・コード生成・書き込み、結果）を JSON のまま1行ずつ表示します。
...
... （Claude が作業するにつれて行が増えます） ...

[GIT] 変更をコミットしています...
[SYSTEM] 完了しました
```

この生ログは透明性・デバッグ用で、読む必要はありません。整形された要約は右ペインに表示されます。**「完了しました」** と表示され、ステータスが再び **「idle」** に戻ったら生成完了です。
...
**Xolvien を再起動するとコンテナも停止します**が、次の操作（指示の送信やテスト実行など）を行うと自動的にコンテナが再起動されます。タスクを削除して作り直す必要はありません。
...
**「rejected」または「failed to push」がログに表示された場合：**
リモートに別の変更がある場合（Reset & Rebuild 後を含む）、push は**自動で解決されます**（取り込み → 再push → 必要に応じてタスク専用ブランチの上書きpush）。ログに「自動解決します...」と表示され、最後に「push 完了」が出れば成功です。

**「変更を反映できませんでした」のエラーバナーが表示された場合：**
自動解決も失敗しています。ログエリアで失敗した手順を確認し、GitHub側でブランチ保護（force-push禁止）が設定されていないか確認してください。

---

## Claude Code CLI の認証エラーが表示された

「**AI の認証に失敗しました**」というエラーバナーが表示された場合、ホストの Claude Code CLI の認証情報（トークン）が失効しています。

ホストの認証情報は **Claude を実行するたびに自動でコンテナへコピーし直される**ため、コンテナ側の操作は不要です。ホストマシンで再ログインするだけで直ります。

```bash
claude --version        # バージョンが表示されるか確認
ls ~/.claude/           # 認証情報ファイルが存在するか確認
claude                  # 起動して /login で再ログイン
```

再ログイン後、アプリでもう一度実行してください。タスクやコンテナを作り直す必要はありません。
